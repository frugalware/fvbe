...
{  
  ...
  strfcpy(command,sizeof(command),"hostnamectl --static set-hostname '%s'",shell_escape(hostname,true));

  if(!execute(command,g->guestroot,0))
    return false;
  
  strfcpy(command,sizeof(command),"hostnamectl --pretty set-hostname '%s'",shell_escape(prettyhostname,true));
