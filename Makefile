...
clean:
	rm -rf $(OBJECTS) src/resolvegroups.o bin/resolvegroups bin/fwsetup root vmlinuz initrd mounts rootfs.img squashfs.img pacman-g2.conf locales layouts fonts $(ISO) tmp local local.lastupdate sums $(FDB) rootfs fvbe.conf var $(ISO_EXTRAS)
